...
final class LoadImageCommentsFromRemoteUseCase: XCTestCase {
    ...
    func test_load_deliversErroOn2xxHTTPResponseWithInvalidJSON() {
        let (sut, client) = makeSUT()
    ...
    private func makeSUT(url: URL =  URL(string: "https://a-givel-url.com")!, file: StaticString = #filePath, line: UInt = #line) -> (sut: RemoteImageCommentsLoader, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
